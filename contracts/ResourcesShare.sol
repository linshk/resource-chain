...
contract Agent {
    ...
    function getProfile() public view returns(string username, uint level, string avatarHash, string motto, uint lastModified) {
        return (profile.username, profile.level, profile.avatarHash, profile.motto, profile.lastModified);
    }
    ...
    function responseResource(uint id, string url, string token, string resourceHash) public onlyOwner
    {
        require(id < requestsCount, "id out of range");

        responses[id] = Response(url, token, block.timestamp, false);
        emit logResponse(requests[id].requester);
    }
    
    // the requester pays for the resource and fetch response for downloading 
    function fetchResponse(uint id) public payable returns (string url, string token, uint timestamp)
    {
        require(id < requestsCount, "id out of range");
        require(requests[id].requester == msg.sender, "access to the request disallowed");
        ...
        emit logFetch(owner);
        return (responses[id].url, responses[id].token, responses[id].timestamp);
    }
